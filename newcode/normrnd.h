...
Pi  0.225260819 0.774739181
**/
void genGMM_layer(int large_layer, int small_max_disparity, bool fullsize = false, double datasetRatio = 0.5) {
    if (!fullsize) { // halfsize
        ...
        if (datasetRatio == 1) {
            if (large_layer == 0) {
                const int k = 4;
                double mu[k] = {0.355655, 51.055870, 4.216459, -15.923241};
                double sigma[k] = {0.656810, 14.776888, 18.815209, 42.960163};
                double pi[k] = {0.676908, 0.076295, 0.075217, 0.171580};
                genGMM(k,mu,sigma,pi,small_max_disparity);
            } else if (large_layer == 1) {
                const int k = 4;
                double mu[k] = {9.154573, 0.398220, 23.247545, -6.074539};
                double sigma[k] = {0.811038,0.644315,6.910081, 21.185447};
                double pi[k] = {0.018760, 0.715488, 0.072520, 0.193232};
                genGMM(k,mu,sigma,pi,small_max_disparity);
            } else { assert(false); }
        } else { assert(false); }
    }
