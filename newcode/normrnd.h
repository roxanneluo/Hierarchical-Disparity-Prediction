...
void genGMM(int K, double const *mu, double const *sigma, double const *pi) {
    int maxGMMDisp = 60;
    length = 2*maxGMMDisp+1;
...
Pi	0.225260819	0.774739181
**/
void genGMM_layer(int large_layer, double datasetRatio =  0.5) {
    if (datasetRatio == 1)
        if (large_layer == 0) {
...
void genGMMRem(int rem) {
    ...
    int K[2] = {2,2};
    genGMM(K[rem],mu[rem], sigma[rem], pi[rem]);
}
