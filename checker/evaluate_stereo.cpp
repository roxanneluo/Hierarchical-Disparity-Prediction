...
bool eval (string result_sha,Mail* mail, int im_num) {

  // ground truth and result directories
  string data_dir = "testdata/kitti_stereo_flow/training";
  string gt_noc_dir = data_dir + "/disp_noc";
  string gt_occ_dir = data_dir + "/disp_occ";
  string gt_img_dir = data_dir + "/image_0";
  string result_dir = "results/kitti_stereo/" + result_sha;
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=3 && argc!=5) {
    cout << "Usage: ./eval_stereo result_sha im_num [user_sha email]" << endl;
    return 1;
  }

  // read arguments
  string result_sha = argv[1];
  int im_num = atoi(argv[2]);
  ...
  // run evaluation
  if (eval(result_sha,mail, im_num)) {
    mail->msg("Your evaluation results are available at:");
    mail->msg("http://www.cvlibs.net/datasets/kitti/user_submit_check_login.php?benchmark=stereo&user=%s&result=%s",argv[3], result_sha.c_str());
  } else {
