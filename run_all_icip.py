...
def check_results(p) :
    ...
    with open(p + 'spec.txt','r') as f :
        para = f.read().split()
    res = subprocess.check_output(['bin/checker_nonocc_red.out',
                                   left_result,
                                   p + 'displeft.pgm',
																	 p + 'dispright.pgm',
                                   tolerance,
...
for who in pic_names :
    ...
    left_result= 'bin/icip2014/' + alg_name + '_' + who + '_left.pgm'
    right_result='bin/icip2014/' + alg_name + '_' + who + '_right.pgm'
