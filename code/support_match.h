...
inline unsigned int GetAddressOffsetImage(const int& u, const int& v,
		const int& width) {
  return v * width + u;
}
...
class SupportMatch {
 public:
	...
	Grid<unsigned char> support_left_map;
	Grid<unsigned char> support_right_map;
  
	SupportMatch (Array3<unsigned char>& left, Array3<unsigned char>& right,
			int w, int h, setting s = MIDDLEBURY) {
	  width = w;
		height = h;
		bpl = width + 15 - (width - 1) % 16;
		
		Grid<unsigned char> left_gray(height, width);
		Grid<unsigned char> right_gray(height, width);

	  // left.rgb_2_gray(left_gray);
		// right.rgb_2_gray(right_gray);	
    ...
    rgb2gray(left_gray, left);
		...
		support_left_map.reset(height, width);
		support_left_map.zero();
		support_right_map.reset(height, width);
		support_right_map.zero();
		para.set(s);
	}
					  ...
					  for (int v_can_2 = v_can - para.incon_window_size;
						  ...
						  if (u_can_2 >= 0 &&
							  ...
							  short d_can_2 = *(D_can +
								...
								if (d_can_2 >= 0 &&
										mylib::ABS(d_can - d_can_2) <= para.incon_threshold)
									support++;
							}
						}
					}
  ...
  // Add if necessary.
    ...
    const int u_step = 2;
      ...
      xmm4 = _mm_load_si128((__m128i*)(I1_block_addr + desc_offset_4));
			...
			int disp_min_valid = mylib::max(para.disp_min, 0);
			int disp_max_valid = para.disp_max;
			if (!right_image) {
			 	disp_max_valid = mylib::min(para.disp_max, u - window_size - u_step);
			} else {
			  disp_max_valid = mylib::min(para.disp_max,
 ...
 private:
	Grid<unsigned char> I1;
	Grid<unsigned char> I2;
	int width;
	int height;
	int bpl;
	parameters para;
