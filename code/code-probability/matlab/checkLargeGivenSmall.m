function checkLargeGivenSmall(HTMLNameSpec,ref, drawflag,prec,folderPrefix)
    name = getNames();
    ...
    for t = 1:times
        for i = 1:dataSetSize
            for range = 1:maxRange
                 if ref
                   dataset = ['Prob_Gen_Ref/Prob_Gen_',name{i},'__'];
                else
...
function [ratio,err]= generateMultiLayer(dataset, dataSetName, maxDisp, level, con, range, smallBase, ref, drawflag,prec,folderPrefix)
    ...
    for i = 1:level
        for j = i+1:level
            A(1:m(j), 1:m(i),j,i) = read([dataset,'cnt_',int2str(j-1),'_',int2str(i-1),'.txt'], m(j), m(i));
        end
    end
...
%             m(large)
%             maxDisp
%             largeGivenSmall = readf(['even_odd_all_large_given_small/',dataSetName,'_LgS.txt'], m(small), m(large));
            
            if ref
               foldername = [folderPrefix,'_ref']; 
            else
               foldername = [folderPrefix,'_no_ref'];
            end
            
            largeGivenSmall = readf([foldername,'/',dataSetName,'_LgS.txt'], m(small), m(large));
...
%             largeGivenSmall(largeGivenSmall>1) = 1;
            ratio(small,large) = getWrongCoverRatio(largeGivenSmall,...
                                    ...
                                    A(1:m(small), 1:m(large), small, large),prec);
...
function IX = getWrongCoverIndex(largeGivenSmall, trueLargeGivenSmall,prec) 
...
function drawAll(A, largeGivenSmall, trueLargeGivenSmall, smallGivenLarge, small, large, dataset, range,ref,prec)
...
%     wrongCover(largeGivenSmall<trueLargeGivenSmall & trueLargeGivenSmall>0) = -1;
    IX = getWrongCoverIndex(largeGivenSmall, trueLargeGivenSmall,prec); 
    wrongCover(IX) = err(IX);
        
    suffix = [int2str(small),int2str(large), 'Range',int2str(range)];
    if ref
        suffix = [suffix ' ref'];
    else
        suffix = [suffix, ' no ref'];
    end
    f = figure();
    [dataset, int2str(f)]
    draw(A, 1,1,1, ['cnt', int2str(small),int2str(large)], false);
    figure();
    draw(smallGivenLarge, 4,2,1, [dataset,suffix], false);
    draw(smallGivenLarge, 4,2,2, [dataset,suffix],false);
    draw(trueLargeGivenSmall, 4,2,3, [dataset,'largeGivenSmall',suffix],false);
    draw(trueLargeGivenSmall, 4,2,4, [dataset,'largeGivenSmall',suffix],false);
    draw(largeGivenSmall, 4,2,5, [dataset,'calculatedLargeGivenSmall',suffix],false);
    draw(largeGivenSmall, 4,2,6, [dataset,'calculatedLargeGivenSmall',suffix],false);
    draw(err, 4,2,7, [dataset,'error',int2str(small),int2str(large), 'Range', int2str(range)],false);
    draw(err, 4,2,8, [dataset,'error',suffix], false);
    figure(); 
...
%             'Rocks2';'Wood1';'Wood2'};
    name = {'Aloe';'Baby1';'Baby2';'Baby3';'Bowling1';'Bowling2';'Cloth1';...
            'Cloth2';'Cloth3';'Cloth4';'Lampshade1';...
            'Lampshade2';'Plastic';'Rocks1';...
            'Rocks2';'Wood1';'Wood2'};
% name = {'Baby1';'Bowling1'};
% name = {'Aloe';'Wood2'};
%     name = {'Aloe';'Baby1';'Baby2';'Baby3';'Bowling1';'Bowling2';'Cloth1';...
