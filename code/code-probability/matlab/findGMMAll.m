function findGMMAll(K, step,times)
    level = 3;
    con = 2;
    smallMax = 60;
    smallM = ones(level,1);
...
%     maxDisp
    ...
    prefix = ['GMM up to ',int2str(K),' gaussians with step ',int2str(step),' for ',int2str(times),' times'];
    f = fopen([prefix,'.html'],'w');
    printHTMLHead(f,prefix);
    for large = 1:level
       for small = large+1:level
           contract = con^(small-large);
            printHTMLTableHead(f,small,large,contract,K);
            for n = 1:N 
                if (n == N)
                    rIndex = dataSetSize;
                else
                ...
                for k = 1:K
                  ...
                  for rem = 0:contract-1
                        ...
                        p = genNormrnd(x,mu(rem+1,:),Pi(rem+1,:),sigma(rem+1,:),k);
                        drawGMM(x,p,distr,small,large,rem,k,KL,maxDisp(lIndex(n)),maxDisp(rIndex));
                   
                  end
                  printHTML(f,mu,Pi,sigma,KL,K,contract);
               end
            end
            fprintf(f,'</tbody>\n</table>\n');
        end
...
function printHTMLTableHead(f,small,large,contract,K)
    fprintf(f,['<h2>d',int2str(small-1),'|d',int2str(large-1),' small base</h2>\n']);
    fprintf(f,'<table border = "3">\n');
    fprintf(f,'<thead>\n<tr><td rowspan=3>disp range</td></tr>\n');
    fprintf(f,'<tr>\n<td>mod</td>\n');
    for rem = 0:contract-1
       fprintf(f,'<td colspan="%d">%d</td>\n',K+2,rem); 
    end
    fprintf(f,'</tr>\n');
    fprintf(f,'<tr>\n<td>K</td>\n');
    for rem = 0:contract-1
...
function printHTML(f,mu,Pi,sigma,KL,K,contract)
    k = size(mu,2);
    if (rem(k,2)==0)
        color = '#e3e3e3';
    else
