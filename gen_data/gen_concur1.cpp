...
string algo, dataset_name, concur_folder, support_folder;
 ...
 * 5: concur_folder, 6: support_folder
 * 7: tot_threshold
 * 8: fullsize/halfsize
 * 9: use_lab
 * 10: data set name
...
int main(int args, char ** argv) {
    ...
    algo = argv[arg_idx++];
    strcpy(file_name[2], argv[arg_idx++]);
  ...
  printf("Loading Gray %s\n", file_name[2]);
    load_image_gray(file_name[2], gnds[0], gnd_h[0], gnd_w[0], scale); // !!!! NOTE: load with scale !!!!
  ...
  printf("Compute Support\n");
    ...
    for (int lvl = 0; lvl < levels-1; ++lvl) {
        support = dpf::getSupportProb(left_pyramid[lvl].rgb, right_pyramid[lvl].rgb, 
