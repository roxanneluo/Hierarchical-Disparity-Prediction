...
BytArray pgm;
Picture ppm;
void ppm2pgm(const char * ppm_file_1, const char * ppm_file_2, 
    char * file_1, char * file_2) {
  const char *ppm_file[2] = {ppm_file_1, ppm_file_2};
  char *file[2] = {file_1, file_2};
  int H, W;
  for (int i = 0; i < 2; ++i) {
    strcpy(file[i], ppm_file[i]);
    file[i][strlen(file[i])-2] = 'g';
    if (!exists(file[i])) {
      load_image(ppm_file[i], ppm, H, W);
      rgb2gray(pgm, ppm, H, W);
      save_image(file[i], pgm, H, W);
/*      string ppm_filename = ppm_file[i];
      string filename = file[i];
      cv::Mat im_gray = imread(ppm_file[i],CV_LOAD_IMAGE_GRAYSCALE);
      imwrite(file[i],im_gray);
      */
    }
  }
}
...
void process (const char* ppm_file_1,const char* ppm_file_2, const char * output_1, int scale = 1) {
  TimeKeeper timer;
  //cout << "Processing: " << file_1 << ", " << file_2 << endl;
  char file_1[256], file_2[256];
  ppm2pgm(ppm_file_1, ppm_file_2, file_1, file_2);
  // load images
  image<uchar> *I1,*I2;
  I1 = loadPGM(file_1);
  I2 = loadPGM(file_2);
  ...
  for (int32_t i=0; i<width*height; i++) {
    ...
    D1->data[i] = (uint8_t)max((double)scale * D1_data[i],0.0);
    D2->data[i] = (uint8_t)max((double)scale * D2_data[i],0.0);
  }
...
void process(const char* file_1, const char * file_2) {
  ...
  output_1[strlen(file_1)-4] = '\0';
  strcat(output_1,"_disp.pgm");
  process(file_1, file_2, output_1);
}
...
int main (int argc, char** argv) {
  ...
  // compute disparity from input pair
    ...
    process(argv[1],argv[2], argv[5], atoi(argv[4]));
//    cout << "... done!" << endl;
